...
class OSS {
  ...
  async getFileUrl(userRelPath: string, prefix?: string): Promise<string> {
    ...
    let url = `/${prefix}/`;
    const envOssUrl = process.env.OSSURL || process.env.ossURL;
    if (envOssUrl && envOssUrl !== "") {
      const normalized = envOssUrl.replace(/\/+$/, "");
      url = normalized.endsWith(`/${prefix}`) ? `${normalized}/` : `${normalized}/${prefix}/`;
    }
